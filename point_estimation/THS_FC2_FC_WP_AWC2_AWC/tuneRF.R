...
for (j in 1:nvar) {
      ...
      if(!dir.exists(file.path(getwd(),"results", var.predict[j]))) {dir.create(file.path(getwd(),"results", var.predict[j]))}
      # 
      ...
      for (i in 1:nl){
            ...
            if(sum(c("CACO3","PH_H2O","CEC")%in%sel.L[[i]]) == 0){
                  ...
                  sel.data <- paste("test", var.predict, sep = "")
              
            } else {
                  sel.data <- paste("test", var.predict, "chem", sep = "")
            }
      ...
      save(CV.rf.L, file = file.path(path_out,var.predict[j],paste(var.predict[j], "_TUNE_200", ".rdata", sep = "")))
      
      # SAVE STATS RESULTS 
      save(tune.res.df, file = file.path(path_out,var.predict[j], paste(var.predict[j], "_TUNE_STATS_200",  ".rdata", sep = "")))
      
      # SAVE VARIMP RESULTS 
      save(varImp.norm.L, file = file.path(path_out,var.predict[j], paste(var.predict[j],"_TUNE_varImp_200", ".rdata", sep = "")))
