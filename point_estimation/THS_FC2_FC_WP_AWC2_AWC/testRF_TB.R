...
for(j in 1:nvar){
      ...
      load(file = file.path(path_out, var.predict[j], paste(var.predict[j], "_BUILT_200", ".rdata", sep = "")))
      ...
      # SAVE STATS RESULTS 
      save(test.stat.df, file = file.path(path_out, var.predict[j], paste(var.predict[j], "_TEST_STATS_200",  ".rdata", sep = "")))
      
      # save raw output of RF predictions on test sets
      save(Test.rf.L, file = file.path(path_out, var.predict[j], paste(var.predict[j], "_Test_200", ".rdata", sep = "")))

      # save residuals, all quantiles, observed value and SAMPLE_ID in one dataframe
      save(Test.res.L, file = file.path(path_out, var.predict[j], paste(var.predict[j], "_TEST_RES_200", ".rdata", sep = "")))
